...
export function useSnapPoints({
    ...
    (height: number) => {
      const newSnapPointIndex =
        snapPointsOffset?.findIndex(
          (snapPointHeight) => snapPointHeight === height
        ) ?? null;
  ...
  function onRelease({
    ...
    const currentPosition = (activeSnapPointOffset as number) - draggedDistance;
    ...
    if (
      ...
      Math.abs(draggedDistance) < window.innerHeight * 0.4
    ) {
      ...
      if (dragDirection > 0 && isLastSnapPoint) {
        snapToPoint(
          snapPointsOffset[(snapPoints as (string | number)[]).length - 1]
        );
        return;
      }
