...
function Root({
  ...
  function shouldDrag(el: EventTarget, isDraggingDown: boolean) {
    ...
    const swipeAmount = drawerRef.current
      ? getTranslateY(drawerRef.current)
      : null;
    const date = new Date();
  ...
  function onDrag(event: React.PointerEvent<HTMLDivElement>) {
    ...
    if (isDragging) {
      ...
      if (!isAllowedToDrag.current && !shouldDrag(event.target, isDraggingDown))
        return;
      (drawerRef.current as HTMLDivElement).classList.add(DRAG_CLASS);
  ...
  React.useEffect(() => {
    if (visible) {
      ...
      const children = (drawerRef.current as HTMLDivElement).querySelectorAll(
        '*'
      );
  ...
  function onNestedOpenChange(o: boolean) {
    ...
    if (!o && drawerRef.current) {
      nestedOpenChangeTimer.current = setTimeout(() => {
        set(drawerRef.current, {
          transition: 'none',
          transform: `translate3d(0, ${getTranslateY(
    ...
    <DialogPrimitive.Content
      onOpenAutoFocus={(e) => {
        if (onOpenAutoFocus) {
          ...
          e.preventDefault();
          (drawerRef.current as HTMLDivElement).focus();
        }
      }}
